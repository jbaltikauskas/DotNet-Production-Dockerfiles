...
<#
.SYNOPSIS
    Builds the Contoso .NET 10 production Docker base images for Ubuntu.
...
.DESCRIPTION
    Top-down flow when this script runs:
        ...
        4. Build Ubuntu images tagged :latest. Both images run unless
    ...
    Images produced for -DotNetVersion 10 (the default):
        ...
        - contoso/ubuntu-net-10:latest                 (target: aspnet-base)

    Dockerfile, build context, and tags use -DotNetVersion:
    dockerfiles/ubuntu/<DotNetVersion> and
    contoso/ubuntu-net-dotnet-tools-<DotNetVersion>:latest.
...
.INPUTS
    None. Pass -DotNetVersion to select a folder other than dockerfiles/ubuntu/10.
...
.EXAMPLE
    PS> .\Image-Build-Ubuntu.ps1
    Builds contoso/ubuntu-net-dotnet-tools-10:latest and contoso/ubuntu-net-10:latest.

.EXAMPLE
    PS> .\Image-Build-Ubuntu.ps1 -DotNetVersion 10
    Builds the Ubuntu images under dockerfiles/ubuntu/10.

.EXAMPLE
    PS> .\Image-Build-Ubuntu.ps1 -NoCache:$false
    Builds both Ubuntu images with BuildKit cache enabled.

.EXAMPLE
    PS> .\Image-Build-Ubuntu.ps1 -ToolsOnly
    Builds only contoso/ubuntu-net-dotnet-tools-10:latest.

.EXAMPLE
    PS> .\Image-Build-Ubuntu.ps1 -WaitOnExit
    Builds both Ubuntu images and waits for Enter before the window closes.
#>
...
try {
    ...
    $versionFolder = "dockerfiles/ubuntu/${DotNetVersion}"
    $dockerfile = "${versionFolder}/Dockerfile"
    $toolsImageTag = "contoso/ubuntu-net-dotnet-tools-${DotNetVersion}:latest"
    $baseImageTag = "contoso/ubuntu-net-${DotNetVersion}:latest"
