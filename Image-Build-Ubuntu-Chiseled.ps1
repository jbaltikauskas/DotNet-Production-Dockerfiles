...
<#
.SYNOPSIS
    Builds the Contoso .NET 10 production Docker base images for Ubuntu Chiseled.
...
.DESCRIPTION
    ...
    Images produced for -DotNetVersion 10 (the default):
        ...
        - contoso/ubuntu-chiseled-net-10:latest                 (target: runtime-base)

    Dockerfile, build context, and tags use -DotNetVersion:
    dockerfiles/ubuntu-chiseled/<DotNetVersion> and
    contoso/ubuntu-chiseled-net-dotnet-tools-<DotNetVersion>:latest.

    The chiseled Dockerfile copies dotnet-tools in the `final` stage. Scratch
    has no shell, so permissions are applied in an earlier chisel stage.
...
.PARAMETER ToolsOnly
    ...
    runtime-base image. Omit it to build both images.
...
.INPUTS
    None. Pass -DotNetVersion to select a folder other than dockerfiles/ubuntu-chiseled/10.
